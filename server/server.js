            ...
            if (user && clientAccount) {
                ...
                if (clientAccount.balance >= transferAmount) {
                    ...
                    if (isUpdate && clientUpdated) {
                        res.status(200).send({ message: "Transfer completed successfully!", success: true });
                    } else {
                        res.status(200).send({ message: "Error updating accounts", success: false });
                    }
                } else {
                    res.status(200).send({ message: "Insufficient balance in client account $" + clientAccount.balance, success: false });
                }
            } else {
                res.status(200).send({ message: "User or client account not found", success: false });
            }
        } else {
            res.status(200).send({ message: "Cannot transfer to the same account", success: false });
        }
    } catch (error) {
        res.status(200).send({ message: "Server error", success: false, error: error.message });
    }
});
...
app.post("/fp", async (req, res) => {
    ...
    if (checkEmail) {
        const updated = await users.findOneAndUpdate({ email }, { password: newPassword })
        res.json({ updated })
    }
})
...
const startServer = async () => {
    mongoose.connect(
        "mongodb+srv://antonyaneric:Erik$2008@cluster0.hfvu6sp.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0/transfer",
    );
    app.listen(3001, () => {
